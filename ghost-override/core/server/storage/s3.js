...
module.exports.save = function(image) {
    ...
    var awsPath = 'http://' + config.bucket + '.s3.amazonaws.com/';
    ...
    .then(function(result) {
        // return unlink(image.path); // Unlinking here breaks /ghost/core/server/api/upload.js
    })
    .then(function() {
        return when.resolve(cdnPath + targetFilename);
    })
    .catch(function(err) {
        // unlink(image.path); // Unlinking here breaks /ghost/core/server/api/upload.js
        errors.logError(err);
        throw err;
    });
};
